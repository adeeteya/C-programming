...
struct node{
  ...
  struct node *prev;//pointer to previous location
}*head;
...
void insert_middle(){
  ...
  if(head==NULL){
    ...
    newnode->next=NULL;
    head=newnode;
    printf("Node inserted at the 1st position as linked list was empty");
  }
  else{
    int i=0,pos,flag=0;
    printf("Enter the position to insert the node(starts from zero): ");
    scanf("%d",&pos);
    if(pos==0){
      newnode->prev=NULL;
      head->prev=newnode;
